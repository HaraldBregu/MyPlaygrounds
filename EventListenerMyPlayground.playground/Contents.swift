...
class Cat {
    ...
    func meow() {
        ...
        self.events.trigger(eventName: "meow", information: "The cat is hungry!");
    }
}
...
class Human {
    func adoptCat(cat:Cat) {
        ...
        cat.events.listenTo(eventName: "meow", action: self.dayDream);
        
        // Using the information from the trigger:
        // (notice the parameters for ponderCat)
        cat.events.listenTo(eventName: "meow", action: self.ponderCat);
    }
